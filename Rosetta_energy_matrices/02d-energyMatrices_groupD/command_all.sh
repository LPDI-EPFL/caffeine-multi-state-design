...
do
  ...
  RESFILE="$BASE_DIRECTORY/01-inputStructs/$state/resfileC"
  rm structure_0001/*
  PARAMS_FILE="$BASE_DIRECTORY/01-inputStructs/$state/CFF.params"
  /Users/pablo/local_lpdi/main/source/bin/ig_dump.macosclangdebug -s $INPUT_STRUCT -resfile $RESFILE -use_input_sc -ex1 -extra_res_fa $PARAMS_FILE 
  cd ..
done
